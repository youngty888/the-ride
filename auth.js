(() => {
  ...
  forms.forEach(form => form.addEventListener('submit', async event => {
    ...
    if (form.dataset.view === 'invite' && values.get('password') !== values.get('confirmPassword')) return message('Passwords do not match.', true);
    ...
    try {
      if (form.dataset.view === 'login') {
        ...
        if (!active?.access_token) throw new Error('This invitation or reset link is missing or expired.');
        await request('/auth/v1/user', { method: 'PUT', token: active.access_token, body: JSON.stringify({ password: values.get('password') }) });
        sessionStorage.removeItem(sessionKey); message('Password saved. Sign in to continue.'); setTimeout(() => show('login'), 900);
      }
    } catch (error) {
      message(error.message === 'SETUP_REQUIRED' ? 'Secure connection setup is incomplete. No information was sent.' : error.message, true);
    } finally { button.disabled = false; }
  }));

  signOut.addEventListener('click', async () => {
    const active = readSession();
    try { if (active?.access_token) await request('/auth/v1/logout', { method: 'POST', token: active.access_token }); } catch {}
    sessionStorage.removeItem(sessionKey); signOut.hidden = true; message('Signed out.');
  });
  ...
  show(['invite', 'recovery'].includes(mode) ? mode : 'login');
