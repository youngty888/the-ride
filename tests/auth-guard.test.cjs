...
test('expired session refreshes once and verifies identity before initialization',async()=>{
 ...
 const ctx=vm.createContext({Date,JSON,atob,AbortSignal,RideSessionStore:{getItem:()=>current,setItem:(k,v)=>current=v},
...
test('failed identity verification never unlocks rider app',async()=>{
 ...
 let destination='';
 const ctx=vm.createContext({Date,JSON,atob,AbortSignal,RideSessionStore:{getItem:()=>JSON.stringify({access_token:token,user:{id:'a'}})},
